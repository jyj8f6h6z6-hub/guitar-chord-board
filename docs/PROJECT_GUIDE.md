...
> 最後更新：2026-08-04
> 適用版本：0.5.0，目前已完成「四種模式、12 種和弦類型、多把位、拖曳式歌曲和弦編排、精簡總覽，以及拍照視譜第一期」的專案
...
- 編排區內的和弦可拖曳重新排序。
- 將編排區內的和弦拖出編排區並放開，即可移除；觸控裝置另有 `×` 按鈕。
- 點擊編排區中的某一個和弦時，才顯示該和弦的完整按法與 12 種同根音類型。
- 選擇類型時，只會替換目前點選的那一個和弦。
- 可替每個編排項目選擇不同把位。
- 可上傳 `PNG、JPG、JPEG、PDF` 歌譜，辨識圖片上已印出的和弦。
- OCR 結果可取代整個和弦編排區，或附加到編排區最後面。
...
目前 OCR 只針對圖片上已印出的和弦文字，不是音符辨識，結果仍須人工確認。
---
...
待選區通常一列顯示 `C～B`。編排區會置中排列和弦，並可拖曳插入、排序或拖出移除。只有點選某一個編排項目後，才會顯示該和弦的詳細區域。
...
- 編排和弦會自動換行。
- 支援觸控拖曳。
- 每個編排項目顯示 `×`，避免手機不易完成「拖出區域」操作。
- 選中和弦的 12 種候選使用兩欄排列。
- 歌譜辨識結果可先在文字框中修正，再取代或附加到編排區。
...
9. 開啟「移除重複和弦」後，重複 symbol 會保留第一次出現的項目。
10. OCR 的「取代和弦編排區」與「加到編排區最後面」正常。
...
確認關閉移除重複時可保留重複項目；開啟後只保留每種和弦第一次出現的位置。
...
- 第 1 至第 11 格多把位。
- 模式三精簡總覽。
- 模式二點選單一和弦後顯示 12 種類型候選。
- 模式二歌譜圖片／PDF 上傳。
- 模式四智慧歌譜第一期。
- Tesseract OCR、PDF.js 與 jsPDF。
- PDF.js Canvas 型別修正。
- OCR 誤判過濾與彩色文字第二辨識嘗試。
- 0.5.0 模式二待選區、拖曳式和弦編排、拖出移除與模式三同步。
- `@dnd-kit` 拖曳套件。
...
目前版本已使用 `0.5.0` 發布拖曳式歌曲和弦編排，因此原先預留給 OMR 的版本號已調整。

下一階段暫定：

```text
0.5.x：模式二拖曳操作、手機體驗與 OCR 匯入細節修正
0.6.0：OMR 與逐小節和聲推演
```
...
- 模式二移除主要的手動文字輸入流程，改為「待選區」與「和弦編排區」。
- 待選區固定保留 `C、D、E、F、G、A、B`。
- 支援點擊加入、拖曳插入、編排區內重新排序。
- 使用 `pointerWithin` 判斷拖出編排區，放開後移除項目。
- 觸控裝置提供 `×` 移除按鈕。
- 每個和弦使用獨立 `SongChordItem.id`，可安全處理重複和弦。
- 只有點選編排區中的和弦時，才顯示該和弦的完整按法與 12 種類型。
- 類型替換與把位選擇以項目 ID 對應，重新排序後不會錯位。
- 模式三同步模式二的順序、重複項目、類型與把位。
- OCR 保留兩個選項：取代編排區、附加到編排區最後面。
...
網站頁尾會透過 `packageJson.version` 顯示：

```text
Guitar Chord Board · 0.5.0
```
